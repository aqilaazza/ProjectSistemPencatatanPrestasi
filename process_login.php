...
if (!$result) {
    // Jika username tidak ditemukan, masukkan log ke tabel login_logs dan redirect ke halaman login
    log_failed_login($username, $role, "Username atau Password Salah!!");
...
if ($dbPassword === $password || password_verify($password, $dbPassword)) {
    ...
    // Jika password salah, masukkan log ke tabel login_logs dan redirect ke halaman login
    log_failed_login($username, $role, "Username atau Password Salah!!");
    header("Location: login.php?message=login_failed");  // Redirect ke halaman login dengan pesan kesalahan
    return;
}

// Fungsi untuk mencatat kegagalan login
function log_failed_login($username, $role, $alasan) {
    $conn = (new connection())->connect();
    $query = "INSERT INTO login_logs (username, role, alasan, waktu_login) VALUES (:username, :role, :alasan, GETDATE())";
    $stmt = $conn->prepare($query);
    $stmt->bindParam(":username", $username);
