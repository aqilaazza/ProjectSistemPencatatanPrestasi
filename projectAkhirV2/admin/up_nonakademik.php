...
// Mengecek parameter di URL
if (isset($_GET['nama_kompetisi']) && !empty($_GET['nama_kompetisi'])) {
    $namakompetisi = htmlspecialchars($_GET['nama_kompetisi']);

    $query = "EXEC up_nonakademik_admin @nama_kompetisi = :nama_kompetisi";
    $stmt = $conn->prepare($query);
    $stmt->bindParam(':nama_kompetisi', $namakompetisi, PDO::PARAM_STR);
    $stmt->execute();
    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    $query = "EXEC up_nonakademik_admin";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $data = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
    ...
    <div class="container">
        ...
        <?php if ($data): ?>
            <?php foreach ($data as $row): ?>
                ...
                <hr> <!-- Tambahkan garis pemisah antara data -->
            <?php endforeach; ?>
        <?php else: ?>
            <p>Data tidak ditemukan untuk kompetisi: <?= htmlspecialchars($namakompetisi ?? 'Semua Kompetisi') ?></p>
        <?php endif; ?>
