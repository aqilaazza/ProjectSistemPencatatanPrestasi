...
// Ambil NIP dari URL, pastikan parameter nip ada
if (isset($_GET['nip'])) {
    $nip = $_GET['nip']; 
} else {
...
$pdo = $koneksi->connect(); // Mendapatkan koneksi PDO

// Query untuk mengambil data jadwal piket berdasarkan NIP
$query = "SELECT nip, hari, jam FROM jadwal_piket WHERE nip = :nip";
$stmt = $pdo->prepare($query);
$stmt->bindParam(':nip', $nip, PDO::PARAM_STR);
$stmt->execute();

// Mengambil data jadwal piket untuk NIP tertentu
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Query untuk mengupdate jadwal piket
    $updateQuery = "UPDATE jadwal_piket SET hari = :hari, jam = :jam WHERE nip = :nip";
    $updateStmt = $pdo->prepare($updateQuery);
    $updateStmt->bindParam(':hari', $new_hari, PDO::PARAM_STR);
    $updateStmt->bindParam(':jam', $new_jam, PDO::PARAM_STR);
    $updateStmt->bindParam(':nip', $nip, PDO::PARAM_STR);
...
<body>
    <div class="container">
        <h2>Ubah Jadwal Piket</h2>
        <form action="#" method="POST">
            <label for="nip">NIP</label>
            <input type="text" name="nama" value="<?php echo htmlspecialchars($nip); ?>" disabled />
            
            <label for="hari">Hari</label>
            <input type="text" name="hari" value="<?php echo htmlspecialchars($hari); ?>" required />

            <label for="jam">Jam</label>
            <input type="text" name="jam" value="<?php echo htmlspecialchars($jam); ?>" required />
